...
//kiem tra di chuyen va loai may Bay
void CheckMoveAndValidateName(string &result, bool &isMove, int &thuTu, bool &isSave,int distance)
{
	int lengh = result.length();
	Gotoxy(X_ADD + distance, thuTu * 3 + Y_ADD);
	cout << result;
...
//So Hieu chuyen bay cho nhap chu va so
void CheckMoveAndValidateID(string &result, bool &isMove, int &thuTu, bool &isSave, int distance)
{
	int lengh = result.length();
	Gotoxy(X_ADD + distance , thuTu * 3 + Y_ADD);
	cout << result;
	...
	while (true)
	{
		while (_kbhit())
		{
			...
			if (key != 224 && key != SPACE && key != 0)
			{
				//ban chu cai in hoa            //bang chu cai in thuong      //so
				if ((key >= 65 && key <= 90) || (key >= 97 && key <= 122)  || (key >= 48 && key <= 57))
				{
					if (count < MAX_LENGTH)
					{
